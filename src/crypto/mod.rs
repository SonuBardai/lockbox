...
use crate::cli::io::{MessageType, print, read_terminal_input};
...
pub fn verify_totp<R: BufRead, W: Write>(reader: &mut R, writer: &mut W, password_store: &mut PasswordStore) -> bool {
...
pub fn totp_init(master_password: &String) {
    ...
    print(&mut output, &*format!("Your totp url is: {}", totp_link), Some(MessageType::Info));
    let _ = TermQrCode::from_bytes(totp_link.as_bytes()).print();
    println!();
    match copy_to_clipboard(totp_link) {
        Ok(_) => {
            print(&mut output, "TOTP link copied to clipboard!", Some(MessageType::Info));
        }
        Err(_) => {}
    };
}